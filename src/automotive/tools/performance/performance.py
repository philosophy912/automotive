...
class Performance(object):
    ...
    def get_qnx_performance(self, port: str, count: int) -> tuple:
        """
        获取QNX的相关性能
        ...
        self.__disconnect()
        return f"{cpu_average}%", f"{percent_average * 100}%", f"{uses_average}M", f"{total_average}M", \
               f"{gpu_average}G"
    ...
    def get_android_performance(self, count: int) -> tuple:
        ...
        return f"{cpu_average}%", f"{round(percent_average * 100, 2)}%", f"{uses_average}M", f"{total_average}M"
