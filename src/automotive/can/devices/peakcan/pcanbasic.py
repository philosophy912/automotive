...
PCAN_ERROR_ANYBUSERR = TPCANStatus(PCAN_ERROR_BUSWARNING |
                                   PCAN_ERROR_BUSLIGHT |
                                   PCAN_ERROR_BUSHEAVY |
                                   PCAN_ERROR_BUSOFF |
...
PCAN_BITRATE_INFO = TPCANParameter(0x18)  # Configured bit rate as btr0_btr1 value
...
class TPCANMsg(Structure):
    ...
    _fields_ = [("id", c_uint),  # 11/29-bit message identifier
                ("msg_type", TPCANMessageType),  # Type of the message
...
class PCANBasic:
    ...
    def __init__(self):
        ...
        if platform.system() == 'Windows':
            if platform.architecture()[0] == '64bit':
                self.__m_dllBasic = windll.LoadLibrary(
                    os.path.split(os.path.realpath(__file__))[0] + r"\x64\PCANBasic.dll")
            elif platform.architecture()[0] == '32bit':
                self.__m_dllBasic = windll.LoadLibrary(
                    os.path.split(os.path.realpath(__file__))[0] + r"\x86\\PCANBasic.dll")
            else:
                self.__m_dllBasic = None
        else:
            self.__m_dllBasic = cdll.LoadLibrary("libpcanbasic.so")
        if self.__m_dllBasic is None:
            logger.info("Exception: The PCAN-Basic DLL couldn't be loaded!")
    ...
    def initialize(
        ...
        Parameters:
          ...
          btr0_btr1 : The speed for the communication (BTR0BTR1 code)
          hw_type   : NON PLUG&PLAY: The type of hardware and operation mode
        ...
        """
        try:
            res = self.__m_dllBasic.CAN_Initialize(channel, btr0_btr1, hw_type, io_port, interrupt)
            return TPCANStatus(res)
        except Exception:
            logger.info("Exception on PCANBasic.initialize")
            raise
    ...
    def initialize_fd(
        ...
        Parameters:
          ...
          bit_rate_fd : The speed for the communication (FD bit rate string)
        ...
        """
        try:
            res = self.__m_dllBasic.CAN_InitializeFD(channel, bit_rate_fd)
            return TPCANStatus(res)
        except Exception:
            logger.info("Exception on PCANBasic.initialize_fd")
            raise
    ...
    def uninitialize(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.uninitialize")
            raise
    ...
    def reset(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.reset")
            raise
    ...
    def get_status(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.get_status")
            raise
    ...
    def read(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.read")
            raise
    ...
    def read_fd(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.read_fd")
            raise
    ...
    def write(
        ...
        Parameters:
          channel      : A TPCANHandle representing a PCAN channel
          message_buffer: A TPCANMsg representing the CAN message to be sent

        Returns:
          A TPCANStatus error code
        """
        try:
            res = self.__m_dllBasic.CAN_Write(channel, byref(message_buffer))
            return TPCANStatus(res)
        except Exception:
            logger.info("Exception on PCANBasic.write")
            raise
    ...
    def write_fd(
        ...
        Parameters:
          channel      : The handle of a FD capable PCAN channel
          message_buffer: A TPCANMsgFD buffer with the message to be sent

        Returns:
          A TPCANStatus error code
        """
        try:
            res = self.__m_dllBasic.CAN_WriteFD(channel, byref(message_buffer))
            return TPCANStatus(res)
        except Exception:
            logger.info("Exception on PCANBasic.write_fd")
            raise
    ...
    def filter_messages(
        ...
        Remarks:
          ...
          If it is desired to reset the filter, please use the 'set_value' function.

        Parameters:
          channel : A TPCANHandle representing a PCAN channel
          from_id  : A c_uint value with the lowest CAN ID to be received
          to_id    : A c_uint value with the highest CAN ID to be received
        ...
        """
        try:
            res = self.__m_dllBasic.CAN_FilterMessages(channel, from_id, to_id, mode)
            return TPCANStatus(res)
        except Exception:
            logger.info("Exception on PCANBasic.filter_messages")
            raise
    ...
    def get_value(
        ...
        """
        try:
            if parameter == PCAN_API_VERSION or \
                    parameter == PCAN_HARDWARE_NAME or \
                    parameter == PCAN_CHANNEL_VERSION or \
                    parameter == PCAN_LOG_LOCATION or \
                    parameter == PCAN_TRACE_LOCATION or \
                    parameter == PCAN_BITRATE_INFO_FD or \
                    parameter == PCAN_IP_ADDRESS:
            ...
            logger.info("Exception on PCANBasic.get_value")
            raise
    ...
    def set_value(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.set_value")
            raise
    ...
    def get_error_text(
        ...
        try:
            ...
            logger.info("Exception on PCANBasic.get_error_text")
            raise
