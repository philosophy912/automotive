...
class CanBus(metaclass=ABCMeta):
    ...
    def __transmit(self, can: CANDevice, message: Message, cycle_time: float):
        """
        CAN发送帧函数，在线程中执行。
        :param can can设备实例化
    ...
    def __cycle_msg(self, can: CANDevice, message: Message):
        """
        发送周期性型号
        :param can can设备实例化
        ...
        if condition1 or condition2:
            ...
            self._transmit_thread.append(self._thread_pool.submit(self.__transmit, can, message, cycle_time))
        else:
    ...
    @staticmethod
    def __event(can: CANDevice, message: Message):
        """
        发送事件信号
        :param can can设备实例化
        :param message: message的集合对象
    ...
    @staticmethod
    def _transmit_one(can: CANDevice, message: Message):
        """
        发送一帧数据
        :param can: can设备实例化
    ...
    def _transmit(self, can: CANDevice, message: Message):
        """
        发送CAN帧函数。
        TODO: 延时没有实现
        :param can: can设备实例化
    ...
    def _resume_transmit(self, can: CANDevice, message_id: int):
        """
        恢复某一帧数据的发送函数。
