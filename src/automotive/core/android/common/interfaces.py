...
from .constants import DEFAULT_TIME_OUT, LOWER_LOCATORS, LOCATORS, LOWER_UISELECTORS, UISELECTORS
...
class BaseAndroid(metaclass=ABCMeta):
    ...
    @abstractmethod
    def close_app(self, package: Optional[str] = None):
        """
        关闭应用
    ...
    @abstractmethod
    def get_element(self, locator: LocatorElement, timeout: float = DEFAULT_TIME_OUT) -> Element:
        """
    ...
    def get_child_element(self,
                          ...
                          locator: Locator,
                          timeout: float = DEFAULT_TIME_OUT) -> Element:
        """
    ...
    @abstractmethod
    def get_element_attribute(self, locator: LocatorElement, timeout: float = DEFAULT_TIME_OUT) -> Attributes:
        """
        获取元素的属性，以列表方式返回
    ...
    @abstractmethod
    def scroll_get_element(self, element: LocatorElement,
                           locator: Locator,
                           text: str,
    ...
    @abstractmethod
    def get_location(self, locator: LocatorElement, timeout: float = DEFAULT_TIME_OUT) -> Position:
        """
    ...
    @abstractmethod
    def click(self, locator: Union[ClickPosition, LocatorElement], timeout: float = DEFAULT_TIME_OUT):
        """
        点击元素的某个位置
    ...
    @abstractmethod
    def press(self, locator: Union[ClickPosition, LocatorElement], duration: float, timeout: float = DEFAULT_TIME_OUT):
        """
        点击元素的某个位置
    ...
    @abstractmethod
    def get_text(self, locator: LocatorElement, timeout: float = DEFAULT_TIME_OUT) -> str:
        """
        获取元素的文本
    ...
    @abstractmethod
    def input_text(self, locator: LocatorElement, text: str, timeout: float = DEFAULT_TIME_OUT):
        """
        对指定控件输入文本信息
    ...
    @abstractmethod
    def clear_text(self, locator: LocatorElement, timeout: float = DEFAULT_TIME_OUT):
        """
        清空编辑框中的文字
    ...
    @staticmethod
    def get_point(start_x: int, start_y: int, width: int, height: int, position: DirectorEnum) -> Tuple[int, int]:
        """
        根据position确定需要点击的范围
    ...
    @staticmethod
    def _get_swipe_point(x: int,
                         y: int,
                         width: int,
                         height: int,
                         direct: SwipeDirectorEnum,
                         swipe_percent: float) -> Position:
        """
        获取要滑动的位置
    ...
    def _get_click_point(self,
                         ...
                         position: DirectorEnum = DirectorEnum.CENTER) -> Tuple[int, int]:
        """
        根据position确定需要点击的范围
    ...
    def _get_element_location(self,
                              locator: Union[ClickPosition, LocatorElement],
                              director: DirectorEnum,
        ...
        :return: x, y
        """
        if not isinstance(locator, (tuple, str, dict, WebElement, UiObject)):
    ...
    def _get_element_text(self, element: Element, timeout: float) -> str:
        """
    ...
    def _scroll_element(self,
                        ...
                        duration: float,
                        direct: SwipeDirectorEnum,
                        swipe_element: Element,
                        locator: dict,
                        text: Optional[str] = None,
                        exact_match: bool = False,
                        timeout: float = None,
                        swipe_time: Optional[int] = None,
                        wait_time: Optional[float] = None) -> Element:
        """
        滑动查找控件: 主要用于文本内容的查找
    ...
    def _get_swipe_param(self,
                         element: LocatorElement,
                         direct: SwipeDirectorEnum,
                         duration: Optional[float] = None,
                         swipe_percent: float = 0.8,
                         check_scrollable: bool = False) -> SwipeParam:
        """
        获取swipe的相关参数
    ...
    @staticmethod
    def _convert_locator(locator: Locator) -> Locator:
        """
        ...
        if isinstance(locator, str):
            ...
            for key, item in locator.items():
                ...
                if lower_key in LOWER_LOCATORS:
                    key = LOCATORS[LOWER_LOCATORS.index(lower_key)]
                elif lower_key in LOWER_UISELECTORS:
                    key = UISELECTORS[LOWER_UISELECTORS.index(lower_key)]
                else:
                    raise TypeError(f"locator type is not in {LOWER_LOCATORS} or {LOWER_UISELECTORS}")
